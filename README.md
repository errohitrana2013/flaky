...
```bash
npm install
npm test          # 55 tests, ~0.4s, no network and no Cloudflare account
...
│
├── tests/
│   └── api.test.mjs          55 tests, no dependencies, runs offline
│
...
```bash
npm test     # 55 tests, no network, no Cloudflare account needed
```
