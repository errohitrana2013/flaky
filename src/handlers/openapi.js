...
export function getOpenApi(ctx) {
  ...
  paths["/custom/{id}/export"] = {
    get: {
      ...
      description:
        "node, python, java and csharp each return a complete single-file server with your data " +
        "embedded — no install, no network, and the chaos parameters work locally exactly as they do " +
        "here, which json-server cannot do. The four servers answer identically, byte for byte. " +
        "json-server returns a db.json; msw returns handlers for a test suite. All six work offline, " +
        "in CI, and after this service is gone.",
      tags: ["custom"],
      parameters: [
        { name: "id", in: "path", required: true, schema: { type: "string" } },
        { name: "format", in: "query", schema: { type: "string", enum: ["node", "python", "java", "csharp", "json-server", "msw"], default: "json-server" } },
      ],
