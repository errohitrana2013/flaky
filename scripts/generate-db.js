...
// Emitted as JSON strings parsed on demand, not as object literals.
//
// Two reasons, both about cold starts. A large JS object literal is parsed by
// the full JavaScript parser when the module loads; JSON.parse is a far simpler
// grammar and measurably faster for the same data. And keeping each collection
// separate means a request for /v1/posts parses ~25 KB rather than the whole
// 346 KB — photos alone was two thirds of it and most requests never touch it.
//
// Counts are emitted alongside so /v1/meta and the landing page can report
// sizes without forcing every collection to parse.
const entries = Object.entries(DB)
  .map(([name, rows]) => `  ${name}: ${JSON.stringify(JSON.stringify(rows))},`)
  .join("\n");
